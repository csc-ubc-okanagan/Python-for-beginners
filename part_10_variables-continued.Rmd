...
Occasionally we need to derive variables form existing information. A good example of this is conversion between scales. If we wanted to change a measurement from metres to kilometres or weight in pounds to kilograms, we could do this simply by performing a basic operation on an entire column. In this situation, let's start by multiplying two columns together to create a new column. Let's say we want to know the total GDP for the country. This could be calculated by multiplying the `population` by the `GDP_per_Cap`.

Again, if you are just starting here, let's re-load in our original Gapminder data:
...
Try creating your own column using either a basic mathematical operation on an existing column, or by combining two columns, or anything else you can think of!
...
Here are the solutions to the examples provided above. If you thought of your own column, that is great!

Solution to #1

```{python, class.source = 'fold-hide', results = TRUE}
